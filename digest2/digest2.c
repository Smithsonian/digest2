...
//                 "%s  observations out of order.", tk->desig + 5);
    ...
    if (obsFirst->ra == obsLast->ra && obsFirst->dec == obsLast->dec) {
        printf("%s tracklet shows no motion.\n", strtok(tk->desig, " "));
//        snprintf(outputLine, outputLineSize,
...
void readMPC80(char *fnObs) {
    ...
    // main loop
    // loops line by line over the obs file...
    while (fgets(line, LINE_SIZE, fobs)) {
        ...
        if (line[14] == 'v') {
            if (tk->lines && parseMpcRoving(line, tk->olist + tk->lines - 1))
                continue;
            pGood = 0;
        } else {
...
/* main
-------------------------------------------------------------------------------
process MPC 80 column or ADES (XML) observation records and output scores to stdout
...
int main(int argc, char** argv) {
    ...
    // if no file specified, just generate model
    if (fnObs == NULL) {
        setup(fnObs);
        return 0;
    }
